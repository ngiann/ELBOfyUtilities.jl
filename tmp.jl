##########################################################
       ...
       ##########################################################
           ...
           PythonPlot.plot(μ[1], μ[2], "x", markersize=12, markeredgewidth=4, color=clr, alpha=alpha)
           PythonPlot.plot(r_ellipse[:,1] .+ μ[1], r_ellipse[:,2] .+ μ[2], clr, linewidth=1, label=label, alpha=alpha)
           ...
           PythonPlot.plot(a_pnts[:,1] .+ μ[1], a_pnts[:,2] .+ μ[2], @sprintf("--%s",clr), linewidth=2, alpha=alpha)
           PythonPlot.plot(b_pnts[:,1] .+ μ[1], b_pnts[:,2] .+ μ[2], @sprintf("--%s",clr), linewidth=2, alpha=alpha)
